...
trait PaymentTrait
{
     ...
     * @return void
     */
    public function processPaymentIntent($request, $settings)
    {
        $order_tracking_number = $request['tracking_number'];
        $order = $this->fetchOrderByTrackingNumber($order_tracking_number);
        if (!$this->paymentIntentExists($order_tracking_number, $settings->options['paymentGateway'])) {
            return $this->savePaymentIntent($order, $settings->options['paymentGateway'], $request);
        } else {
            return PaymentIntent::where('tracking_number', '=', $order_tracking_number)->where('payment_gateway', '=', strtoupper($settings->options['paymentGateway']))->first();
        }
    }
     ...
     * paymentIntentExists
     *
     * @param  mixed $order_tracking_number
     * @param  mixed $payment_gateway
     * @return object
     */
    public function paymentIntentExists($order_tracking_number, $payment_gateway)
    {
     ...
     * @return array
     */
    public function createPaymentIntent($order, $request, $payment_gateway): array
    {
